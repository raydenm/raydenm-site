...
export function DateDetails({ date }: { date: Date }) {
  ...
  return (
    <div className="scrollbar-hide h-[536px] overflow-auto text-sm">
      <div className="flex flex-col items-center">
        ...
        <div className="relative my-2 flex size-20 items-center justify-center rounded-xl bg-blue-500 pt-3 text-5xl font-semibold text-white">
          {solar.getDay()}
          <div
            className="absolute left-2 top-2 size-2 rounded-full bg-white
            "
          ></div>
          <div
            className="absolute right-2 top-2 size-2 rounded-full bg-white
            "
          ></div>
        </div>
        <div>
      ...
      <div className="mt-4 flex flex-col text-muted-foreground">
        <div className="flex items-start border-t border-muted py-2">
          <div className="mr-3 shrink-0 rounded-[4px] bg-pink-300 px-[5px] py-0.5 text-xs text-white">星座</div>
          <div>{solar.getXingZuo()}座</div>
        </div>
        {festivals && festivals.length > 0 && (
          <div className="flex items-start border-t border-muted py-2">
            <div className="mr-3 shrink-0 rounded-[4px] bg-blue-600 px-[5px] py-0.5 text-xs text-white">节日</div>
            <div>{festivals.join(', ')}</div>
          </div>
        )}
        <div className="flex items-start border-t border-muted py-2">
          <div className="mr-3 shrink-0 rounded-[4px] bg-green-600 px-[5px] py-0.5 text-xs text-white">宜</div>
          <div>{lunar.getDayYi().join(', ')}</div>
        </div>
        <div className="flex items-start border-t border-muted py-2">
          <div className="mr-3 shrink-0 rounded-[4px] bg-red-500 px-[5px] py-0.5 text-xs text-white">忌</div>
          <div>{lunar.getDayJi().join(', ')}</div>
        </div>
        <div className="flex border-t border-muted py-2">
          <div className="flex w-1/2 items-start">
            <div className="mr-3 shrink-0 rounded-[4px] bg-slate-600 px-[5px] py-0.5 text-xs text-white">月相</div>
            <div>{lunar.getYueXiang()}月</div>
          </div>
          <div className="flex w-1/2 items-start">
            <div className="mr-3 shrink-0 rounded-[4px] bg-zinc-600 px-[5px] py-0.5 text-xs text-white">物候</div>
            <div>{lunar.getWuHou()}</div>
          </div>
        </div>
        <div className="flex flex-col items-start border-t border-muted py-2">
