...
export const NavigationLink = memo(({ href, label, icon, shortcutNumber }: NavigationLinkProps) => {
  ...
  if (!isInternal) {
    return (
      <a
        ...
        className="flex items-center justify-between gap-2 rounded-lg p-2 transition-all hover:bg-muted/50"
      >
  ...
  return (
    <Link
      ...
      href={href}
      className={cn(
        'group flex items-center justify-between rounded-lg p-2 transition-all',
        isActive ? 'bg-primary text-primary-foreground' : 'hover:bg-muted/50'
      )}
    >
      <span className="flex items-center gap-2">
        {iconCmp}
        <span className={cn('font-medium', isActive && 'text-primary-foreground')}>{label}</span>
      </span>
      {shortcutNumber && (
        <span
          className={cn(
