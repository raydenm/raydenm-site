 ...
 */
export const getDateTimeFormat = (date: string | number | Date) => {
 ...
 */
export const sortByProperty = cache((arr: any[], prop: string | number) => {
  return arr?.sort((a: { [x: string]: string }, b: { [x: string]: string }) => {
...
export const getSortedPosts = cache((posts: any[]) => {
  return posts.sort(
    (a: { date: any; sys: { firstPublishedAt: any } }, b: { date: any; sys: { firstPublishedAt: any } }) => {
 ...
 */
export const dateWithMonthAndYearFormatter = Intl.DateTimeFormat('zh-CN', {
  month: '2-digit',
  year: 'numeric',
  timeZone: 'UTC'
})
 ...
 */
export const dateWithDayAndMonthFormatter = Intl.DateTimeFormat('zh-CN', {
  day: '2-digit',
  month: '2-digit',
  timeZone: 'UTC'
})
...
export const getItemsByYear = (items: any[]) => {
  return items.reduce((acc, item) => {
    ...
    const yearArr = acc.find((item: number[]) => item[0] === year)
    if (!yearArr) {
      acc.push([year, [item]])
    } else {
      yearArr[1].push(item)
